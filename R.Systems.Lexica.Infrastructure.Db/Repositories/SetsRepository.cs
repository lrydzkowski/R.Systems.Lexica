    ...
    : IGetSetsRepository, IGetSetRepository, IDeleteSetRepository, ICreateSetRepository, IUpdateSetRepository
{
    ...
    public async Task<bool> SetExistsAsync(string setName, long? ignoreSetId, CancellationToken cancellationToken)
    {
        bool setExists = await _dbContext.SetEntities.AsNoTracking()
            .Where(x => x.Name == setName && (ignoreSetId == null || x.SetId != ignoreSetId))
            .Select(x => x.Name)
